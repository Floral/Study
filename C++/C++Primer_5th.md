...
> C++11中新增了”右值引用“，会在后面介绍，其主要用于内置类。严格来说，当我们使用术语”引用（reference）“时，指的是”左值引用“。
...
> 因此，我们在设计一些**常量对象也能使用的成员函数**时，需要将他们声明为常量成员函数。
...
每个输出流都管理一个缓冲区，用来保存对象程序读写的数据。例如，如果执行下面的代码：
...
由于需要析构函数的类都是包含指针或者分配了动态内存的类，因此拷贝、赋值的时候需要分配新的内存，否则合成的拷贝构造、赋值函数会将指针指向的地址赋给新的对象。导致两个对象的成员指向同一块地址。（如果你就是要这样的效果。。。当我没说）
...
如果声明为成员函数，那么这个运算符的第一个运算对象就会被隐式地绑定到这个函数的this指针上。因此，该重载的运算符函数的参数列表的参数数量总比该运算符的操作数的数量少一个。同时要注意，成员函数版本的运算符的左侧运算对象必须是运算符所属的一个对象。

外部函数形式的重载运算符的参数数量就等于该运算符的操作数的数量，且按顺序对应。

不过，我们也有一些标准来选择哪一种形式更好：

- 赋值、下标、调用和成员访问箭头运算符必须是成员函数。
- 复合值运算符一般来说是成员，但非必须。
- 改变对象状态的运算符或者与给定类型密切相关的运算符，如递增、递减和解引用运算符，通常是成员。
- 具有对称性的运算符可能转换任一端的运算对象，例如算数、相等性、关系和位运算符，一般定义成非成员函数。
...
同时，有一些运算符虽然可以重载，但是我们不应该重载，因为重载后的运算符不能保证运算对象的求值顺序（&&、||这类），或者是已经被打上了深深的烙印，如果重载的话会让用户很不适应（例如逗号运算符和取地址运算符）。
...
如果要重载多个关系运算符的话（==、<、>），那么就要保证这些运算符的评判标准是统一的，不能出现相互矛盾的情况。

不然就会反逻辑233。

##### 递增和递减运算符

因为递增和递减运算符只有一个运算对象，因此按照常规的方法定义这两种是一样的函数，为了区分开来，我们将前置运算符还是按照原来的规则定义，后置运算符的参数列表加上一个int类型的形参（不被使用），只用声明int类型，不用命名。

##### 成员访问运算符

成员访问运算符包括解引用运算符（*）和箭头运算符（->），这两种的操作有点难以区分和理解，我找了一个解释的比较不错的博客，大家可以看一下：

https://blog.csdn.net/francisapp/article/details/58044233

##### 重载函数调用运算符

函数调用运算符就是`()`运算符，如果我们对一个类重载这个运算符，那么这个类就能被当做一个函数一样来调用。

对于定义了调用运算符的类，我们称之为“**函数对象(function object)**”。

其实，我们在之前也定义过函数对象，也就是lambda表达式。编译器会将我们写的lambda表达式翻译成一个未命名的类的未命名对象，这个类中有一个重载的函数调用运算符。

**标准库定义的函数对象**

标准库也在functional头文件中定义了一些表示算术运算符、关系运算符和逻辑运算符的类（P509）。可用于算法中。
...
调用形式就是返回值和参数列表的集合，不同调用对象可以有同一个调用形式，例如：`int(int, int)`表示了返回值为int，参数为两个int的一类调用对象。

由于函数指针不能指向重载了函数调用运算符的类和lambda表达式，不能表示所有的可调用对象。标准库头文件functional中定义了一个function的类来用于存储这些可调用对象。（P512）



#### 类型转换

类型转换运算符是一种特殊的成员函数，它负责将一个类类型的值转换成其他类型。即一个类被当做参数传给不同参数的可调用对象时，这个类可以转换成不同的类型（如果定义了相应的类型转换运算符的话）。

定义类型转换运算符的形式比较特别：`operator type() const `，其中type是你想要转换成的目标类型，const是因为这个运算符通常不应该修改原对象的值。

类型转换一般会由编译器自动完成，但是有时会引起不必要的误会，因此，为抑制隐式转换，可以加上explicit强制显式用`static_cast<type>(object)`转换，但是当这个对象作为条件时，也能被隐式转换，这是一个特例。（P516）



#### 重载运算符可能带来的问题

重载类型转换运算符可能会导致二义性的问题。（P517）

- 如果A类定义了参数为B类的构造函数，而B类重载了转换成A类的类型转换运算符，那么就可能会有产生二义性的情况。
- 一个类定义了多个参数为单个内置类型的构造函数和类型转换运算符时。
- 重载的普通函数对不同的重载了类转换运算符的类
- ......
...
输出结果是：I’m A!。如果p的声明类型是B&，那么输出结果是：I’m B!。这里基类A的func方法并没有加virtual关键字。如果我们加上virtual，那么结果就会变成，无论p的声明是A&还是B&，输出结果都是：I’m B!。

但是如果我们在这里直接通过对象本身a或b调用他们的func方法，那么结果就是调用它们各自类型中定义的func，也就是a.func()输出“I’m A!”。b.func()输出“I’m B!”。

于是我们可以总结一下，virtual只是对于**使用引用**（**指针**也行，大家可以自行实验）**来调用父类子类的方法**有影响。在父类的func前加上virtual的情况下，通过指针（或引用）调用该方法，实际调用的是该指针所指向的实际的类中定义的方法。这也就是**动态绑定**的意思，也就是在运行的过程中动态确定指针所指向的实际对象是什么（是父类还是子类）并调用实际对象的方法。相对的，静态绑定就是仅仅根据声明的引用或指针的类型来决定调用的方法，即静态绑定到了声明对象的类型上。
