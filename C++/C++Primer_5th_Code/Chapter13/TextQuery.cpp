...
TextQuery::TextQuery(ifstream& infile):data(new StrVec)
{
    string line;
    size_t line_no = 1;
    size_t times = 0;
...
QueryResult::QueryResult(const string found_word,
                ...
                const size_t times):found_word(found_word),data(data),lines(lines),times(times){};
...
ostream& print(ostream& os,const QueryResult &result)
{
    ...
    {
        os<<"(line "<<(*it)<<") ";
        os<<(result.data->begin())[*it-1]<<std::endl;
        it++;
    }
    return os;
}