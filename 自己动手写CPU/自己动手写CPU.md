...
由于这个相关问题是在mem阶段才读取到新的要加载的值，但是在id阶段已经读取了原始值，当前一条指令处于mem阶段时，后一条指令已经处于执行阶段了，所以数据前推的方法也不能解决。因此我们需要在id模块新增一个判断当前指令与前一条指令是否存在load相关的部分，如果存在相关，则暂停一个时钟周期，延长译码阶段，然后再将访存阶段的最新数据前推到译码阶段。具体实现请查看本书Chapter9_1中id.v模块新增的判断代码以及本书9.10小节。
