...
为了实现暂停，我们新增一个CTRL模块，用于接收暂停请求信号和发送暂停信号。由于只可能在译码和执行阶段需要多个周期，所以只有译码和执行两个阶段可以发送暂停请求。当一个阶段请求暂停时，这个阶段之前的流水线都得暂停，之后的可以继续执行。另外，译码和执行这两个阶段不需要接受暂停信号，且接受暂停的都是各个模块之间的过度模块和pc_reg.v（因为他们才受时钟周期的控制）。